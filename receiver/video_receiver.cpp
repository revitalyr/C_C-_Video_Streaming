...
{
    m_logger->info(LogFormat("Initializing VideoReceiver with config: port={}, jitter_buffer_size={}", 
    ...
    try {
        ...
        m_logger->info(LogFormat("VideoReceiver initialized successfully"));
    } catch (const std::exception& e) {
        m_logger->error(LogFormat("Failed to initialize VideoReceiver: {}", e.what()));
        throw;
    }
}
...
bool VideoReceiver::start() {
    if (m_running.load()) {
        m_logger->warn(LogFormat("VideoReceiver is already running"));
        return false;
    }
    ...
    try {
        ...
        if (!m_socket->open()) {
            m_logger->error(LogFormat("Failed to open UDP socket"));
            return false;
        }
        
        if (!m_socket->bind(bind_endpoint)) {
            m_logger->error(LogFormat("Failed to bind UDP socket to {}:{}", 
        ...
        m_receiver_thread = std::thread(&VideoReceiver::receiver_loop, this);
        m_running = true;
        
        m_logger->info(LogFormat("VideoReceiver started successfully on {}:{}", 
            m_config.bind_ip, m_config.port));
        return true;
    } catch (const std::exception& e) {
        m_logger->error(LogFormat("Failed to start VideoReceiver: {}", e.what()));
        return false;
    }
}
...
void VideoReceiver::stop() {
    ...
    m_logger->info(LogFormat("Stopping VideoReceiver..."));
    
    m_stop_requested = true;
    m_frame_queue_cv.notify_all();
    ...
    m_logger->info(LogFormat("VideoReceiver stopped. Final stats: frames={}, packets={}, loss={:.2f}%, fps={:.2f}", 
        final_stats.frames_received, final_stats.packets_received, 
        final_stats.packet_loss_rate * 100, final_stats.fps_actual));
}

std::unique_ptr<VideoFrame> VideoReceiver::get_frame(int timeout_ms) {
...
void VideoReceiver::receiver_loop() {
    m_logger->info(LogFormat("VideoReceiver thread started"));
    ...
    while (!m_stop_requested) {
        try {
            ...
            if (bytes_received > 0) {
                packet_buffer.resize(bytes_received);
                process_rtp_packet(packet_buffer);
                packet_buffer.resize(m_config.max_frame_size);
            ...
            m_logger->error(LogFormat("Error in receiver loop: {}", e.what()));
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }
    }
    
    m_logger->info(LogFormat("VideoReceiver thread stopped"));
}
...
std::unique_ptr<VideoFrame> VideoReceiver::assemble_frame(
    ...
    // Установка метаданных (эвристические значения)
    frame->width = 1920;  // Можно парсить из SPS
    frame->height = 1080; // Можно парсить из SPS
